...
variable "project_id" {
  description = "Specifies the GCP Project ID that will contain all created resources. Default is empty."
  type        = string
  default     = ""
}
...
variable "instance_count" {
  description = "Specifies the number of GCP Compute Engine instances (nodes) to create for the k3s cluster. This value defines the total cluster size, including the first server node, additional server nodes (if count <= 3), and worker nodes (if count > 3). Default is '1'."
  type        = number
  default     = 1
  validation {
...
variable "ai_factory_enabled" {
  ...
  validation {
    condition = (
      var.ai_factory_enabled == false ||
      var.rancher_enabled == true
    )
    error_message = "When ai_factory_enabled is true, enabled_enabled must also be true."
  }
}
