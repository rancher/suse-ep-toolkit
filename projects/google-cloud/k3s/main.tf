...
resource "ssh_resource" "retrieve_kubeconfig" {
  ...
  commands = [
    ...
    "sudo cat /etc/rancher/k3s/k3s.yaml | sed -e 's/127.0.0.1/${module.k3s_first_server.instances_public_ip[0]}/g' -e '/certificate-authority-data:/c\\    insecure-skip-tls-verify: true'"
  ]
