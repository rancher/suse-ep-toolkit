...
variable "instance_count" {
  description = "Specifies the number of Droplets (nodes) to create for the K3s cluster. This value defines the total cluster size, including the first server node, additional server nodes (if count <= 3), and worker nodes (if count > 3). Default is '1'."
  type        = number
  default     = 1
  validation {
...
variable "image_id" {
  description = "Specifies the ID of the custom OS image used to provision all K3s cluster droplets. Default is empty."
  type        = string
  default     = ""
}
